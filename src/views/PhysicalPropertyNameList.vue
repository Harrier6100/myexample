    ...
    <div class="container-fluid">
        ...
        <div class="mb-3">
            ...
            <table class="table table-bordered table-hover">
                <thead>
                    <tr>
                        <th :class="orderBy('code')" @click="sortBy('code')">物性コード</th>
                        <th :class="orderBy('name')" @click="sortBy('name')">物性名</th>
...
const fetchItems = async () => {
    try {
        ...
        const response = await api.get(`api/physical/property/names`);
...
const removeItem = async (id) => {
    if (confirm('削除しますか？')) {
        try {
            store.commit('startLoading');
            const response = await api.delete(`/api/physical/property/names/${id}`);
